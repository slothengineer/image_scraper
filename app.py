...
def index():
    if request.method == 'POST':
                try:
                    ...
                    for index,image_tag in enumerate(image_tags):
                                ...
                                with open(os.path.join(save_directory, f"{query}_{image_tags.index(image_tag)}.jpg"), "wb") as f:
                                    f.write(image_data)
                    client = pymongo.MongoClient("mongodb+srv://slothengineer:slothengineer@cluster0.ufeklbo.mongodb.net/?retryWrites=true&w=majority")
                ...
                # Code that can potentially raise an exception
                      ...
                      return "An error occurred while making the request to Google Images."
                except pymongo.errors.PyMongoError as e:
                        logging.error(f"PyMongo error occurred: {str(e)}")
                        return "An error occurred while inserting data into MongoDB."
